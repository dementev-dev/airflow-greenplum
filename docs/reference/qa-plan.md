# История QA: план отладки полной цепочки

Этот план сохраняет сценарии первоначальной проверки полной реализации:
повторную загрузку, несколько учебных дней и граничные случаи.
Он предназначен для подготовки углубленной диагностики на отдельном тестовом
стенде. Ожидания и SQL ниже нужно сверять с текущей веткой; учебная `main`
содержит заглушки и не обязана проходить проверки полной реализации.

Для своего задания используйте [самопроверку](../assignment/README.md#запуск-и-проверка).
Действующие команды разработки и границы автоматических проверок описаны
в [TESTING.md](../../TESTING.md), устройство DQ - в [справочнике](dq_taxonomy.md).

Сценарии ниже очищают DWH и изменяют данные. `make bookings-init`
пересоздает базу Bookings, а `make dwh-truncate` удаляет строки хранилища.
Выполняйте их только на стенде, где данные можно потерять.
Параметры доступа к Airflow берите из `.env`.
