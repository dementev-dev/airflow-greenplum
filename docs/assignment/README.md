...
Сначала вы восстановите прием справочника в STG, затем дополните цепочку
загрузками ODS, измерениями DDS и четырьмя витринами DM.
Требования к этим расширениям описаны в [ТЗ](analyst_spec.md).
...
## Прием справочника: PXF → STG

Пройдите [«Восстановите прием справочника из источника»](pxf_airports.md).
Вы разберете готовый участок `airports`, восстановите внешнюю таблицу
и загрузку. На повторном запуске и переименовании аэропорта выясните,
как загрузка сохраняет снимки источника.

## Первая загрузка: ODS

После приема справочника реализуйте `ods.airplanes`, затем `ods.seats`.
...
В заданиях ODS, DDS и DM вы заменяете эти заглушки; менять DAG
и создавать таблицы заново не требуется.
...
Для первых загрузок ODS этого разбора достаточно.
...
1. Используйте общий снимок после восстановления источника
   в лабораторной PXF. Новый запуск STG для этой работы не нужен.
