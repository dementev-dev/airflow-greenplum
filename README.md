...
загрузки **bookings → stg** и **stg -> ods** в Greenplum, чтобы вы могли сфокусироваться на DWH‑части
(ODS/DDS/DM) и не тратить время на инфраструктуру.
...
## Быстрый старт (основной сценарий: bookings -> stg -> ods)
...
4) Подготовьте STG/ODS-объекты в Greenplum (выберите один вариант):

- Учебный вариант: в Airflow UI запустите DAG `bookings_stg_ddl`, затем `bookings_ods_ddl`;
- Технический шорткат: `make ddl-gp` (применяет DDL для STG и ODS разом вручную).
...
make ddl-gp             # применить DDL STG+ODS к Greenplum вручную (вместо DDL-DAG)
