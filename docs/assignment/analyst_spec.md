...
Программа начинается с [приема справочника через PXF](pxf_airports.md).
Затем переходите к расширениям из этого ТЗ; порядок их запуска и проверки
находится во [входе в задания](README.md#первая-загрузка-ods). В каждом разделе ниже есть
...
> тоже эталонный. В первой лабораторной вы временно разбираете и восстанавливаете
> прием `airports`; расширения по этому ТЗ начинаются с ODS: `airplanes` и `seats`.
...
После лабораторной PXF разберите [загрузку `ods.airports`](../design/reading_the_pipeline.md#ods-airports).
...
6. **SCD1-обновление денормализованных полей:** Обновите `departure_city`,
   `arrival_city`, `airplane_model`, `total_seats` только у открытых версий
   (`valid_to IS NULL`), даже если `hashdiff` не менялся. Новая версия при этом
   не создается. Закрытые версии сохраняют значения на момент закрытия.
...
После реализации слоя следуйте [порядку запуска и проверки](README.md#запуск-и-проверка).
В `bookings_validate` смотрите группу своего этапа:

- `validate_ods` проверяет данные `airplanes` и `seats`, их ключи
  и согласованность с выбранным снимком STG.
- `validate_dds` проверяет наличие данных в учебных измерениях, уникальность
  пассажиров и сценарий изменения маршрута SCD2 с восстановлением таблиц.
- `validate_dm` проверяет, что четыре учебные витрины содержат строки.

Этот DAG дополняет ваши `*_dq.sql`. В частности, непустота витрины еще
не подтверждает формулы метрик: проверяйте их по требованиям выше и
[классам DQ](../reference/dq_taxonomy.md). Перед следующей загрузкой дождитесь
всего DAG и успешного `validate_dds.scd2_restore`.
