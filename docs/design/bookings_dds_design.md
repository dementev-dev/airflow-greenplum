# История проектирования DDS для Bookings

> Архивный материал: исходный план создания слоя. Он сохраняет обоснования,
> варианты SQL и этапы разработки; приведенные фрагменты отражают состояние
> проекта во время проектирования.

Для работы с текущей учебной веткой откройте
[руководство загрузки DDS](../bookings_to_gp_dds.md).
Смысл строк и связи показаны на [карте](architecture-map.html#node=dds.fact_flight_sales);
поля и правила самостоятельных загрузок определяет [ТЗ](../assignment/analyst_spec.md).
Готовые примеры доступны из [разбора SQL](reading_the_pipeline.md).

Ниже сохранен план "DDS Layer: план реализации Star Schema для bookings".
Задачи по созданию файлов относятся к истории разработки, а примеры SQL
требуют сверки с действующими файлами перед использованием.
