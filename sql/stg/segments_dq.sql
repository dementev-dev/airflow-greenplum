...
BEGIN
    ...
    IF v_src_count = 0 THEN
        ...
        IF v_stg_count <> 0 THEN
            RAISE EXCEPTION
                'DQ FAILED: источник segments_ext за окно инкремента пустой, но в stg.segments есть строки текущего _load_id (_load_id=%): %',
                v_batch_id,
                v_stg_count;
        END IF;

        RAISE NOTICE
            'В источнике segments_ext нет строк для окна инкремента (book_date > %). Пропускаем DQ проверки (_load_id=%).',
    ...
    IF v_orphan_ticket_count <> 0 THEN
        RAISE EXCEPTION
            'DQ FAILED: найдены segments без соответствующих tickets (_load_id=%): %',
            v_batch_id,
    ...
    IF v_orphan_flight_count <> 0 THEN
        RAISE EXCEPTION
            'DQ FAILED: найдены segments без соответствующих flights (_load_id=%): %',
            v_batch_id,
            v_orphan_flight_count;
    END IF;

    RAISE NOTICE
        'DQ PASSED: segments ок (_load_id=%): source=% stg=%',
        v_batch_id,
        v_src_count,
        v_stg_count;
END $$;
