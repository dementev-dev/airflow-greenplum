...
После [запуска стенда](../README.md):

1. [Структура DWH](design/db_schema.md): смысл строк и связей на примере поездки.
2. [Учебные задания](assignment/README.md): прием справочника через PXF,
   затем загрузки ODS, измерения и витрины. Каждая работа указывает свои проверки.

Перед заданиями ODS прочитайте [разбор готового справочника](design/reading_the_pipeline.md#ods-airports).
Сборка факта из того же руководства понадобится перед DDS.
Поля и бизнес-правила расширений находятся в [ТЗ](assignment/analyst_spec.md).
